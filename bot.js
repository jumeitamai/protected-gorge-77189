...
controller.hears(['挨拶', 'こんにちは', 'Bot', 'あなた', '誰', 'だれ', '自己紹介'], 'direct_message,direct_mention,mention', function (bot, message) {
    ...
    bot.reply(message, '宮田農園怠惰担当　ペテルギウス・ミヤタコンティ...*デスッ！* \n _あぁ、脳が震える_ :smiley:');
...
controller.hears(['ちん'], 'direct_message,direct_mention,mention', function (bot, message) {
 ...
 controller.hears(['脳'], 'direct_message,direct_mention,mention', function (bot, message) {
    ...
    // bot.reply()で、botに発言をさせます。
    bot.reply(message, 'あぁ脳が震える震える震える...震えるぅぅぅぅぅ!! :mushroom:');
...
controller.hears(['好きな女性のタイプ'], 'direct_message,direct_mention,mention', function (bot, message) {
    ...
    bot.startConversation(message, function (err, convo) {
        ...
        // convo.ask() で質問をします。
        convo.ask('私の好きな女性のタイプを当ててみてください', [
            {
                pattern: 'すず', // マッチさせる単語
                callback: function (response, convo) {
                    ...
                    convo.say('正解！:ok_woman:\nペロペロしたい（ハァハァal:+1:'); // convo.say()で発言をします。
